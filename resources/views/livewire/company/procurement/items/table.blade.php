<div>
    <x-alerts.flash />
    <div class="table-responsive" wire:key="items-table-{{ $requestId ?? 'req' }}">
    ...
    $sym = '₹';
                    ...
                    <tr>
                        ...
                        <td class="text-truncate" style="max-width: 260px;" title="{{ $it->name }}">{{ $it->name }}</td>
                        <td class="text-capitalize">{{ $it->priority ?? '—' }}</td>
                        <td>{{ (int) ($it->quantity ?? 1) }}</td>
                        <td>{{ $it->unit ?? '—' }}</td>
                        <td>{{ optional($it->date_required)->format('Y-m-d') ?? '—' }}</td>
